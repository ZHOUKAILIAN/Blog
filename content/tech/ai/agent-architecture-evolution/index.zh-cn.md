...
## 🧭 时间线双列视角：从 Prompt 启发到架构工程化

同一条时间线，用架构视角再读一遍“谁推动谁”：左边是 Prompt 先点亮的能力，右边是系统把它稳定化、产品化，最后再反向影响模型能力。

| 时间线 | Prompt 先点亮的能力 | 架构做成稳定功能 | 关键关系（读者视角） |
| --- | --- | --- | --- |
| 零样本 / 少样本 | **Zero-shot / Few-shot**：靠指令/示例控制输出格式 | **早期工具调用雏形**：用 Few-shot 强制输出 JSON | 架构把“可控输出”变成接口 |
| CoT | **Chain-of-Thought**：让模型写出推理过程 | **Planning / ReAct Loop**：推理+行动流程 | 架构把“想清楚”变成“可执行” |
| Self-Consistency | **多路径采样 + 投票**：选更稳答案 | **鲁棒决策机制**：多路评估/验证 | 架构把“多样性”转成“稳定性” |
| ReAct | **Thought/Action/Observation** 提示模板 | **Tool Use + Agentic Loop**：有执行体的闭环 | 架构实现协议与执行体 |
| Function Calling | **Few-shot 逼近结构化调用** | **Function Calling 微调**：系统基石 | 架构推动能力内化到模型 |
| Toolformer | **模型自监督学工具**（学术探索） | **工业界用外部 Schema**（更可控、易扩展） | 架构在可控性上做取舍 |
| MRKL | **“该找谁”意识**（路由/分发） | **Router / Orchestrator** 多模块协作 | 架构把意图变成模块治理 |
| ToT / GoT | **树/图式推理**：可回溯与合并 | **搜索与规划架构**（BFS/DFS/MCTS） | 架构把思维变成搜索 |
| SoT | **先大纲后展开**：提示并行写作 | **并行生成管线**：降低延迟 | 架构把并行策略做成流水线 |
| Reflexion | **反思与自我修正** | **Memory / Feedback Loop**：长期记忆 | 架构把反思固化为记忆 |
| RAG | **检索增强提示**：外部资料注入 | **检索管线 + 向量库**：系统组件 | 架构把检索变成能力 |
| 多智能体 | **角色分工提示**：分角色协作 | **Multi-Agent 编排**：工程化协作 | 架构把协作变成组织结构 |
